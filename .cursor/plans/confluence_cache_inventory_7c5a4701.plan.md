...
**08:00 OI prefetch** ([`settings.yaml`](confluence-configs/settings.yaml): SPY, QQQ, SMH, IGV, IWM, HACK, MEME, XLE, XLI) writes **disk only** via [`prefetchWatchlist`](internal/confluence/loop.go) — no processor memory.

**Startup bootstrap (separate path):** On processor start outside RTH, [`bootstrapWatchlistIfClosed`](internal/confluence/bootstrap.go) runs a full `BootstrapSnapshot` for every prefetch symbol → up to **9 full snapshots** land in `snapshots` (with slices in `tickerRuntime`) **without registry entries**, until each ticker is deactivated or overwritten. This is the largest confluence-only RAM spike at idle startup.
